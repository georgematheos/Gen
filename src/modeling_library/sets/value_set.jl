...
struct UniqueValueSet <: CustomUpdateGF{PersistentSet, UniqueValueSetState} end
...
function apply_with_state(::UniqueValueSet, (dict,))
    vals = PersistentSet(values(dict))
    (vals, UniqueValueSetState(vals, dict))
end
function update_with_state(::UniqueValueSet, st, (dict,), (diff,)::Tuple{DictDiff})
    set = st.vals
    added = Set()
    deleted = Set()
